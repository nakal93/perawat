...
<div class="min-h-screen bg-gray-50">
    ...
    <div class="bg-white shadow-sm border-b">
        <div class="w-full">
            <div class="flex justify-between items-center h-14 sm:h-16">
                <h1 class="text-lg sm:text-xl font-semibold text-gray-900">Profil Karyawan</h1>
                <!-- Status Badge - Mobile optimized -->
                <div class="flex items-center">
                    @php $status = auth()->user()->status; @endphp
                    <span class="inline-flex items-center px-2 sm:px-4 py-1 sm:py-2 rounded-full text-xs sm:text-sm font-medium {{ $status==='active' ? 'bg-emerald-50 border border-emerald-200 text-emerald-700' : ($status==='approved' ? 'bg-blue-50 border border-blue-200 text-blue-700' : 'bg-amber-50 border border-amber-200 text-amber-700') }}">
                        <span class="hidden sm:inline">Status: </span>{{ ucfirst($status) }}
                    </span>
                </div>
            </div>
        </div>
            ...
            <div class="p-4">
                <div class="flex flex-col gap-4">
                    ...
                    <div class="flex justify-center">
                        <div class="relative w-20 h-20 shrink-0">
                            <div class="w-full h-full rounded-xl border-2 border-gray-100 bg-gray-50 overflow-hidden flex items-center justify-center">
                                @if($fotoUrl && file_exists(public_path('storage/'.ltrim($karyawan->foto_profil,'/'))))
                                    <img src="{{ $fotoUrl }}" alt="Foto Profil" class="w-full h-full object-cover cursor-pointer" onclick="openImageModal('{{ $fotoUrl }}', 'Foto Profil - {{ auth()->user()->name }}')">
                                @else
                    ...
                    <div class="text-center">
                        <h1 class="text-xl font-semibold text-gray-900 mb-2">{{ $karyawan->gelar ? auth()->user()->name . ' ' . $karyawan->gelar : auth()->user()->name }}</h1>
                        <p class="text-gray-600 mb-4 text-sm">{{ auth()->user()->email }}</p>
                        <div class="flex flex-wrap justify-center gap-2">
                            @if($karyawan->statusPegawai)
                                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-blue-50 text-blue-700 border border-blue-200">{{ $karyawan->statusPegawai->nama }}</span>
                            @endif
                            @if($karyawan->ruangan)
                                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-purple-50 text-purple-700 border border-purple-200">{{ $karyawan->ruangan->nama_ruangan }}</span>
                            @endif
                            @if($karyawan->profesi)
                                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-amber-50 text-amber-700 border border-amber-200">{{ $karyawan->profesi->nama_profesi }}</span>
                            @endif
                        </div>
                    </div>
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                        ...
                        @foreach($fieldsIdentitas as $f)
                            <div class="p-3 sm:p-4 rounded-lg bg-gray-50 border border-gray-100 hover:bg-gray-100 transition-colors">
                                <dt class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">{{ $f['label'] }}</dt>
                                <dd class="text-sm font-semibold text-gray-900 break-words">{{ $f['value'] }}</dd>
                            </div>
                        @endforeach
                        <div class="sm:col-span-2 p-3 sm:p-4 rounded-lg bg-gray-50 border border-gray-100 hover:bg-gray-100 transition-colors">
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6">
                    ...
                    <div class="grid grid-cols-1 gap-3 sm:gap-4">
                        ...
                        @foreach($fieldsKontak as $f)
                            <div class="p-3 sm:p-4 rounded-lg bg-gray-50 border border-gray-100 hover:bg-gray-100 transition-colors">
                                <dt class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">{{ $f['label'] }}</dt>
                                <dd class="text-sm font-semibold text-gray-900 break-all">{{ $f['value'] }}</dd>
                            </div>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6">
                    ...
                    <div class="grid grid-cols-1 gap-3 sm:gap-4">
                        ...
                        <div class="p-3 sm:p-4 rounded-lg bg-gray-50 border border-gray-100 hover:bg-gray-100 transition-colors">
                            ...
                            <dd class="font-mono text-xs bg-gray-200 rounded px-2 py-1 text-gray-700 select-all break-all">{{ $karyawan->qr_token }}</dd>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
            ...
            @if(auth()->user()->status === 'active')
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6">
                    ...
                    <div class="p-3 sm:p-4 bg-gray-50 rounded-lg flex items-center justify-center">
                        @if($qrcode)
                            <div class="max-w-32 max-h-32">{!! $qrcode !!}</div>
                        @else
            ...
            <form method="POST" action="{{ route('karyawan.profile.update') }}" class="space-y-6" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="nik" id="nik" value="{{ old('nik', $karyawan->nik) }}" required class="w-full h-10 sm:h-12 rounded-lg border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500">
                        @error('nik')<p class="mt-2 text-sm text-red-600">{{ $message }}</p>@enderror
                    </div>

                    <!-- NIP -->
                    <div>
                        <label for="nip" class="block text-sm font-medium text-gray-700 mb-2">NIP <span class="text-xs text-gray-500">(Wajib untuk PNS/PPPK)</span></label>
                        <input type="text" name="nip" id="nip" value="{{ old('nip', $karyawan->nip) }}" class="w-full h-10 sm:h-12 rounded-lg border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500">
                        <p class="mt-1 text-xs text-gray-500">Wajib untuk status PNS atau PPPK.</p>
                    ...
                    <div>
                        ...
                        <select name="jenis_kelamin" id="jenis_kelamin" required class="w-full h-10 sm:h-12 rounded-lg border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal_lahir" id="tanggal_lahir" value="{{ old('tanggal_lahir', $karyawan->tanggal_lahir) }}" required class="w-full h-10 sm:h-12 rounded-lg border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <div>
                        ...
                        <select name="golongan_darah" id="golongan_darah" class="w-full h-10 sm:h-12 rounded-lg border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <div>
                        ...
                        <select name="status_perkawinan" id="status_perkawinan" class="w-full h-10 sm:h-12 rounded-lg border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500">
                            <option value="">Pilih</option>
                    ...
                    <div>
                        ...
                        <input type="text" name="no_hp" id="no_hp" value="{{ old('no_hp', $karyawan->no_hp) }}" class="w-full h-10 sm:h-12 rounded-lg border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500" placeholder="0812...">
                    ...
                    <div>
                        ...
                        <input type="text" name="nama_ibu_kandung" id="nama_ibu_kandung" value="{{ old('nama_ibu_kandung', $karyawan->nama_ibu_kandung) }}" class="w-full h-10 sm:h-12 rounded-lg border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <div>
                        ...
                        <select name="status_pegawai_id" id="status_pegawai_id" required class="w-full h-10 sm:h-12 rounded-lg border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500">
                            <option value="">Pilih</option>
                    ...
                    <div>
                        ...
                        <select name="ruangan_id" id="ruangan_id" required class="w-full h-10 sm:h-12 rounded-lg border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500">
                            <option value="">Pilih</option>
                    ...
                    <div>
                        ...
                        <select name="profesi_id" id="profesi_id" required class="w-full h-10 sm:h-12 rounded-lg border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500">
                            <option value="">Pilih</option>
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal_masuk_kerja" id="tanggal_masuk_kerja" value="{{ old('tanggal_masuk_kerja', $karyawan->tanggal_masuk_kerja) }}" class="w-full h-10 sm:h-12 rounded-lg border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <div>
                        ...
                        <select name="agama" id="agama" required class="w-full h-10 sm:h-12 rounded-lg border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <div>
                        ...
                        <input type="text" name="pendidikan_terakhir" id="pendidikan_terakhir" value="{{ old('pendidikan_terakhir', $karyawan->pendidikan_terakhir) }}" required class="w-full h-10 sm:h-12 rounded-lg border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500" placeholder="contoh: S1 Keperawatan">
                    ...
                    <div>
                        ...
                        <input type="text" name="gelar" id="gelar" value="{{ old('gelar', $karyawan->gelar) }}" class="w-full h-10 sm:h-12 rounded-lg border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500" placeholder="S.Kep, Ners">
                    ...
                    <div>
                        ...
                        <input type="text" name="kampus" id="kampus" value="{{ old('kampus', $karyawan->kampus) }}" class="w-full h-10 sm:h-12 rounded-lg border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500" placeholder="Universitas ...">
                ...
                <div class="pt-6 border-t border-gray-200 flex flex-col sm:flex-row sm:items-center gap-4">
                    <button type="submit" class="w-full sm:w-auto px-6 sm:px-8 py-3 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg text-sm transition-colors focus:ring-4 focus:ring-blue-200">
                        Simpan & Selesaikan
                    </button>
    ...
    <!-- Desktop: Full width container (mepet kiri-kanan) -->
    <div class="w-full py-4 sm:py-6 lg:py-8 space-y-6 sm:space-y-8 hidden lg:block">
        @php($isComplete = ($karyawan->status_kelengkapan === 'lengkap'))
            ...
            <div class="p-4 sm:p-6 lg:p-8">
                <div class="flex flex-col lg:flex-row lg:items-center gap-6 lg:gap-10">
                    ...
                    <div class="flex justify-center lg:justify-start">
                        <div class="relative w-20 h-20 sm:w-24 sm:h-24 lg:w-32 lg:h-32 shrink-0">
                            <div class="w-full h-full rounded-xl border-2 border-gray-100 bg-gray-50 overflow-hidden flex items-center justify-center">
                                @if($fotoUrl && file_exists(public_path('storage/'.ltrim($karyawan->foto_profil,'/'))))
                                    <img src="{{ $fotoUrl }}" alt="Foto Profil" class="w-full h-full object-cover cursor-pointer" onclick="openImageModal('{{ $fotoUrl }}', 'Foto Profil - {{ auth()->user()->name }}')">
                                @else
                            ...
                            <div class="absolute -bottom-1 -right-1 w-5 h-5 sm:w-6 sm:h-6 bg-emerald-500 rounded-full flex items-center justify-center border-2 border-white">
                                <svg class="w-2.5 h-2.5 sm:w-3 sm:h-3 text-white" viewBox="0 0 20 20" fill="currentColor">
                    ...
                    <div class="flex-1 min-w-0 text-center lg:text-left">
                        <h1 class="text-xl sm:text-2xl lg:text-3xl font-semibold text-gray-900 mb-2">{{ $karyawan->gelar ? auth()->user()->name . ' ' . $karyawan->gelar : auth()->user()->name }}</h1>
                        <p class="text-gray-600 mb-4 text-sm sm:text-base">{{ auth()->user()->email }}</p>
                        <div class="flex flex-wrap justify-center lg:justify-start gap-2">
                            @if($karyawan->statusPegawai)
                                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-blue-50 text-blue-700 border border-blue-200">{{ $karyawan->statusPegawai->nama }}</span>
                            @endif
                            @if($karyawan->profesi)
                                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-indigo-50 text-indigo-700 border border-indigo-200">{{ $karyawan->profesi->nama_profesi }}</span>
                            @endif
                            @if($karyawan->ruangan)
                                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium bg-purple-50 text-purple-700 border border-purple-200">{{ $karyawan->ruangan->nama_ruangan }}</span>
                            @endif
                    ...
                    <div class="flex justify-center lg:justify-end">
                        <a href="{{ route('karyawan.settings') }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg border border-gray-300 text-gray-700 text-sm font-medium hover:bg-gray-50 transition-colors">
                            ...
                            <span class="hidden sm:inline">Edit </span>Profil
                        </a>
                    </div>
                </div>
            </div>
            ...
            @if(auth()->user()->status === 'active')
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6">
                    ...
                    <div class="flex items-center justify-center">
                        @if($qrcode)
                            <div class="max-w-28 max-h-28 bg-white p-2 rounded border border-gray-200">{!! $qrcode !!}</div>
                        @else
                            <div class="text-gray-500 text-sm">QR Code tidak tersedia</div>
                        @endif
                    </div>
        ...
        @else
        <!-- Belum Lengkap: Form Lengkapi Data -->
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6 lg:p-8">
            <div class="mb-6 sm:mb-8">
                <h3 class="text-lg sm:text-xl font-semibold text-gray-900 mb-2">Lengkapi Profil Anda</h3>
                <p class="text-sm sm:text-base text-gray-600">Isi seluruh field wajib bertanda <span class="text-red-500 font-semibold">*</span> untuk mengaktifkan akun. Setelah disimpan, tampilan akan berubah menjadi ringkasan.</p>
            </div>
            ...
            <form method="POST" action="{{ route('karyawan.profile.update') }}" class="space-y-6 sm:space-y-8" enctype="multipart/form-data">
                @csrf
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                    ...
                    <div>
                        <label for="nik" class="block text-sm font-medium text-gray-700 mb-2">NIK *</label>
                        <input type="text" name="nik" id="nik" value="{{ old('nik', $karyawan->nik) }}" required maxlength="16" class="w-full h-10 sm:h-12 rounded-lg border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500" placeholder="16 digit">
                        @error('nik')<p class="mt-2 text-sm text-red-600">{{ $message }}</p>@enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="nip" id="nip" value="{{ old('nip', $karyawan->nip) }}" class="w-full h-10 sm:h-12 rounded-lg border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500" placeholder="18 digit">
                        @error('nip')<p class="mt-2 text-sm text-red-600">{{ $message }}</p>@enderror
                    </div>
                    ...
                    <div>
                        <label for="golongan_darah" class="block text-sm font-medium text-gray-700 mb-2">Golongan Darah *</label>
                        <select name="golongan_darah" id="golongan_darah" required class="w-full h-10 sm:h-12 rounded-lg border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500">
                            <option value="">Pilih</option>
                            @foreach(['A','B','AB','O'] as $gol)
                                <option value="{{ $gol }}" @selected($karyawan->golongan_darah===$gol)>{{ $gol }}</option>
                            @endforeach
                        </select>
                        @error('golongan_darah')<p class="mt-2 text-sm text-red-600">{{ $message }}</p>@enderror
                    </div>
                    ...
                    <div>
                        <label for="status_perkawinan" class="block text-sm font-medium text-gray-700 mb-2">Status Perkawinan *</label>
                        <select name="status_perkawinan" id="status_perkawinan" required class="w-full h-10 sm:h-12 rounded-lg border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500">
                            <option value="">Pilih</option>
                            @foreach(['Belum Kawin','Kawin','Cerai Hidup','Cerai Mati'] as $stat)
                                <option value="{{ $stat }}" @selected($karyawan->status_perkawinan===$stat)>{{ $stat }}</option>
                            @endforeach
                        </select>
                        @error('status_perkawinan')<p class="mt-2 text-sm text-red-600">{{ $message }}</p>@enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="tel" name="no_hp" id="no_hp" value="{{ old('no_hp', $karyawan->no_hp) }}" required class="w-full h-10 sm:h-12 rounded-lg border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500" placeholder="08xxxxxxxxxx">
                        @error('no_hp')<p class="mt-2 text-sm text-red-600">{{ $message }}</p>@enderror
                    </div>
                    ...
                    <div>
                        <label for="nama_ibu_kandung" class="block text-sm font-medium text-gray-700 mb-2">Nama Ibu Kandung *</label>
                        <input type="text" name="nama_ibu_kandung" id="nama_ibu_kandung" value="{{ old('nama_ibu_kandung', $karyawan->nama_ibu_kandung) }}" required class="w-full h-10 sm:h-12 rounded-lg border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500">
                        @error('nama_ibu_kandung')<p class="mt-2 text-sm text-red-600">{{ $message }}</p>@enderror
                    </div>

                    <!-- Alamat: Provinsi s/d Detail -->
                    <div class="sm:col-span-2 lg:col-span-3">
                        <label class="block text-sm font-medium text-gray-700 mb-2">Alamat Domisili *</label>
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
                            <select id="provinsi_id" name="provinsi_id" required class="w-full h-10 sm:h-12 rounded-lg border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500" data-selected="{{ $karyawan->provinsi_id }}">
                                <option value="">Provinsi</option>
                            </select>
                            <select id="kabupaten_id" name="kabupaten_id" required class="w-full h-10 sm:h-12 rounded-lg border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500" data-selected="{{ $karyawan->kabupaten_id }}">
                                <option value="">Kabupaten</option>
                            </select>
                            <select id="kecamatan_id" name="kecamatan_id" required class="w-full h-10 sm:h-12 rounded-lg border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500" data-selected="{{ $karyawan->kecamatan_id }}">
                                <option value="">Kecamatan</option>
                            </select>
                            <select id="kelurahan_id" name="kelurahan_id" required class="w-full h-10 sm:h-12 rounded-lg border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500" data-selected="{{ $karyawan->kelurahan_id }}">
                                <option value="">Kelurahan</option>
                            </select>
                        </div>
                        <textarea id="alamat_detail" name="alamat_detail" rows="2" class="w-full rounded-lg border-gray-300 text-sm focus:ring-blue-500 focus:border-blue-500" placeholder="Detail (RT/RW, Jalan, Blok)">{{ old('alamat_detail', $karyawan->alamat_detail) }}</textarea>
