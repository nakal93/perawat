                        ...
                        <div class="space-y-1">
                            ...
                            <a href="{{ route('admin.karyawan.index') }}" class="@if(request()->routeIs('admin.karyawan.*')) bg-gradient-to-r from-green-50 to-green-100 text-green-700 border-r-4 border-green-600 shadow-sm @else text-slate-700 hover:bg-slate-50 hover:text-slate-900 @endif group flex items-center px-3 py-3 text-sm font-medium rounded-xl transition-all duration-200 relative overflow-hidden">
                                ...
                                <div class="relative flex items-center w-full">
                                    ...
                                    <span class="font-medium">Data Komkep</span>
                                </div>
                            </a>
                            ...
                            <div class="pt-6 mt-6">
                                ...
                                <!-- Security (submenu) -->
                                <div x-data="{ openSecurity: {{ request()->routeIs('admin.security.*') || request()->routeIs('telescope*') ? 'true' : 'false' }} }" class="mt-2">
                                    <button @click="openSecurity = !openSecurity" class="group flex items-center justify-between w-full px-3 py-2.5 text-sm font-medium rounded-xl transition-all duration-200 hover:bg-slate-50 text-slate-700">
                                        <div class="flex items-center">
                                            <div class="w-7 h-7 rounded-lg flex items-center justify-center mr-3 transition-colors bg-slate-100 text-slate-500 group-hover:bg-slate-200">
                                                <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        ...
                                        <svg class="w-4 h-4 text-slate-400 transition-transform duration-200" :class="openSecurity ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                                        </svg>
                                    </button>
                                    <div x-show="openSecurity" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 -translate-y-1" class="ml-4 mt-2 space-y-1">
                                        <!-- Laravel Telescope -->
                                        <a href="/telescope" target="_blank" class="@if(request()->routeIs('telescope*')) bg-blue-50 text-blue-700 @else text-slate-600 hover:text-slate-800 hover:bg-slate-50 @endif group flex items-center px-3 py-2 text-xs font-medium rounded-lg transition-all duration-200">
                                            <div class="w-2 h-2 bg-blue-400 rounded-full mr-3"></div>
                                            <span>Laravel Telescope</span>
                                            <svg class="w-3 h-3 ml-auto text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/>
                                            </svg>
                                        </a>
                                        <!-- System Logs -->
                                        <a href="{{ route('admin.security.logs') }}" class="@if(request()->routeIs('admin.security.logs')) bg-amber-50 text-amber-700 @else text-slate-600 hover:text-slate-800 hover:bg-slate-50 @endif group flex items-center px-3 py-2 text-xs font-medium rounded-lg transition-all duration-200">
                                            <div class="w-2 h-2 bg-amber-400 rounded-full mr-3"></div>
                                            <span>System Logs</span>
                                        </a>
                                        <!-- Activity Logs -->
                                        <a href="{{ route('admin.security.activity') }}" class="@if(request()->routeIs('admin.security.activity')) bg-emerald-50 text-emerald-700 @else text-slate-600 hover:text-slate-800 hover:bg-slate-50 @endif group flex items-center px-3 py-2 text-xs font-medium rounded-lg transition-all duration-200">
                    ...
                    <div class="flex items-center h-16 px-6 bg-gradient-to-br from-blue-600 via-blue-700 to-blue-800 relative overflow-hidden">
                        ...
                        <div class="relative flex items-center justify-between w-full">
                            <div class="flex items-center space-x-3">
                                ...
                                <div>
                                    <h1 class="text-white font-bold text-lg tracking-tight">RSUD Dolopo</h1>
                                    <p class="text-blue-100 text-xs font-medium">Area Komite Keperawatan</p>
                                </div>
                            </div>
                            <button type="button" id="mobile-close-button" class="lg:hidden p-2 rounded-lg text-white/80 hover:text-white hover:bg-white/10 transition-colors">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="lg:pl-64">
                    ...
                    <div class="sticky top-0 z-40 flex h-16 shrink-0 items-center gap-x-4 border-b border-slate-200 bg-white px-4 shadow-sm sm:gap-x-6 sm:px-6 lg:px-8">
                        <div class="flex items-center">
                            ...
                            <span class="ml-2 text-base font-semibold text-slate-900">Area Komite Keperawatan</span>
                        </div>
