@extends('layouts.app')
@section('breadcrumb','Tambah Status Pegawai')
@section('content')
<div class="max-w-md mx-auto bg-white border rounded-xl p-6">
    <h1 class="text-lg font-semibold mb-4">Tambah Status Pegawai</h1>
    <form method="POST" action="{{ route('admin.status-pegawai.store') }}" class="space-y-4">
        @csrf
        <div>
            <label class="block text-sm text-slate-700">Nama Status</label>
            <input type="text" name="nama" value="{{ old('nama') }}" required class="mt-1 w-full border rounded-lg px-3 py-2" />
